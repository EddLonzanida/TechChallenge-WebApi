...
{
    public abstract class CrudControllerSoftDeleteBase<TKey, T> : ApiControllerBase
        where T : class, IEntityBase<TKey>, IEntitySoftdeletableBase, ISearchableName
    {
        protected readonly IDataRepositorySoftDeleteBase<TKey, T> repository;
        ...
        protected virtual async Task<T> DeleteItem(TKey id, string deletionReason = "")
        {
            return await repository.DeleteAsync(id, deletionReason);
        }
        ...
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete([FromRoute] TKey id, [FromBody] string deletionReason = "")
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var item = await DeleteItem(id, deletionReason);
